...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function getSigVisible() {
    return window.innerWidth < 480 ? 1 : window.innerWidth < 768 ? 1 : 3;
  }
  ...
  function updateSig(animate = true) {
    ...
    const outerW  = sigTrack.parentElement.offsetWidth;
    const cardW   = outerW / visible;
    const maxIdx  = Math.max(0, sigCards.length - visible);
  ...
  document.getElementById('sigPrev').addEventListener('click', () => { sigIndex--; updateSig(); });
  document.getElementById('sigNext').addEventListener('click', () => { sigIndex++; updateSig(); });
  ...
  window.addEventListener('resize', () => { setSigSizes(); updateSig(false); });
